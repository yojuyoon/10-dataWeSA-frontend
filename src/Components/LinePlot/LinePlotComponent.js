...
function LinePlotComponent(props) {
  const [optionData, setOptionData] = useState(options);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [selectedCategory, setselectedCategory] = useState("Grocery and Pharmacy");
      ...
      <Container>
        <LeftContainer>
          ...
          <LeftCategory>
            ...
            <select onChange={(e) => handleCategory(e)} className="PlaceCategory">
              {categories.map((place, key) => (
                <option value={key} selected={selectedCategory === place && "selected"}>
                  {place}
                </option>
              ))}
            </select>
